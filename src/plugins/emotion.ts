 ...
 * 转换成图片表情
 */
function toEmotion(text: string) {
  ...
  if (!text) return text;
  text = text.replace(/\[[\u4E00-\u9FA5]{1,3}\]/gi, function (word: string) {
    ...
    const imgHTML = `<img class="static-emotion-gif" style="vertical-align: middle" src="${path}/emotion/${index}.gif" alt="">`
    return imgHTML;
  });
  return text;
}

export const emotion: Directive = {
  mounted(el, binding) {
